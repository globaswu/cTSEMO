...
This document describes the solver implementation distributed in `src/`.
Curated manuscript results are stored separately under
...
`src/vendor/bradford-tsemo/`. They are pinned to upstream commit
...
Run `setup_ctsemo` from the repository root. The public interface is
...
Executable examples are stored in `src/examples/`, benchmark definitions in
`src/benchmarks/`, diagnostic utilities in `src/diagnostics/`, and class-based
tests in `src/tests/`. Refer to the MATLAB help text for the exact option schema and
...
DIRECT, NGPM, and MEX directories. See the repository-root
...
> E. Bradford, A. M. Schweidtmann, and A. Lapkin, “Efficient
...
BSD 2-Clause license under `src/vendor/bradford-tsemo/LICENSE`.
