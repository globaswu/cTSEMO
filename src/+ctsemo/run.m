...
function selection = selectNextPoint( ...
    ...
    primarySearch = primarySearchNotRun( ...
        options, numel(primaryRows), iteration, ...
        "no_feasible_observation");
    fallbackReason = "";
    fallbackUsed = false;
    ...
    if ~hasFeasibleObservation
        ...
        referenceAcquisitionStatus = acquisitionStatus;
        referenceEpsilon = components.epsilon;
        ...
        canRunPrimaryGA = options.primarySearch.method == "ga" && ...
            ~generatorFailed && ...
            ~logical(referenceAcquisitionStatus.requiresFallback) && ...
            isfinite(verifiedMaxAF);
        if canRunPrimaryGA
            fixedOptions = options;
            fixedOptions.acquisition.epsilon = referenceEpsilon;
            fixedOptions.acquisition.backgroundScale = 0;
            scoreFunction = @(XUnit) primaryAcquisitionScore( ...
                XUnit, objectiveDraws, pofModel, feasibleY, ...
                acquisitionReferencePoint, ...
                normalizeInputs(data.X, problem.lowerBound, ...
                    problem.upperBound), ...
                standardizedTrainingY, objectiveCenter, ...
                objectiveScale, fixedOptions);
            [primaryUnit, ~, primarySearch] = ...
                ctsemo.optimizePrimaryGA( ...
                scoreFunction, candidate.XUnit(primaryRows, :), ...
                candidate.AF(primaryRows), options, iteration);
            optimizedPools = primaryGaPools( ...
                pools, primaryUnit, problem, primarySearch);
            primaryYDraw = evaluateDraws(objectiveDraws, primaryUnit);
            candidate = concatenateCandidates( ...
                optimizedPools, primaryYDraw, 1, challengerYDraw);
            [candidate.pof, candidate.rawPof, ...
                    candidate.pofDiagnostics] = ...
                predictFeasibilityField(pofModel, candidate.XUnit);
            candidate.YDrawStandardized = ...
                (candidate.YDraw - objectiveCenter) ./ objectiveScale;
            [candidate.AF, components, acquisitionStatus] = ...
                ...
                candidate.pof, feasibleY, acquisitionReferencePoint, ...
                normalizeInputs(data.X, problem.lowerBound, ...
                    problem.upperBound), ...
                standardizedTrainingY, fixedOptions);
            acquisitionStatus.epsilonSource = ...
                "initial_primary_and_challenger_pools";
            acquisitionStatus.referencePositiveHVISampleCount = ...
                referenceAcquisitionStatus.positiveHVISampleCount;
            candidate.sampledHVI = components.hvi;
            candidate.epsilon = components.epsilon;
            candidate.designMask = components.designMask;
            candidate.codomainMask = components.codomainMask;
            ...
            candidate.hardDuplicate = components.hardDuplicate;
            candidate.fallbackScore = nan(size(candidate.AF));
            if ~logical(referenceAcquisitionStatus.requiresFallback) && ...
                    any(candidate.AF > 0)
                acquisitionStatus.requiresFallback = false;
                acquisitionStatus.reason = "ok";
            end
            primaryRows = find(candidate.source == "primary");
            challengerRows = find(candidate.source == "challenger");
            [primaryMaxAF, primaryIndex] = verifiedMaximum( ...
            ...
            [challengerMaxAF, challengerIndex] = verifiedMaximum( ...
                candidate.AF, challengerRows);
            verifiedMaxAF = max(primaryMaxAF, challengerMaxAF);
        elseif options.primarySearch.method == "pool"
            primarySearch = primarySearchNotRun( ...
                options, numel(primaryRows), iteration, ...
                "primary_pool_requested");
            primarySearch.status = "pool";
            primarySearch.seedBestScore = primaryMaxAF;
            primarySearch.bestScore = primaryMaxAF;
            primarySearch.improvementOverSeed = 0;
            primarySearch.returnedSeedBest = true;
        else
            primarySearch = primarySearchNotRun( ...
                options, numel(primaryRows), iteration, ...
                string(referenceAcquisitionStatus.reason));
        end
    ...
    selection.primarySearch = primarySearch;
    ...
    selection.summary = struct( ...
        ...
        "primarySearchStatus", primarySearch.status, ...
...
function score = primaryAcquisitionScore( ...
        XUnit, objectiveDraws, pofModel, feasibleY, referencePoint, ...
        Xtrain, Ytrain, objectiveCenter, objectiveScale, options)
    YDraw = evaluateDraws(objectiveDraws, XUnit);
    YDrawStandardized = (YDraw - objectiveCenter) ./ objectiveScale;
    [pof, ~, ~] = predictFeasibilityField(pofModel, XUnit);
    score = ctsemo.scoreCandidates( ...
        XUnit, YDrawStandardized, pof, feasibleY, referencePoint, ...
        Xtrain, Ytrain, options);
end

function pools = primaryGaPools(pools, primaryUnit, problem, diagnostics)
    primaryUnit = reshape(primaryUnit, 1, []);
    primaryX = problem.lowerBound + primaryUnit .* ...
        (problem.upperBound - problem.lowerBound);
    origin = "ga";
    if diagnostics.status == "poolFallback"
        origin = "primary_pool_fallback";
    elseif diagnostics.returnedSeedBest
        origin = "primary_seed";
    end
    pools.primary = struct( ...
        "X", primaryX, ...
...
function diagnostics = primarySearchNotRun( ...
        options, seedCount, iteration, reason)
    ...
    diagnostics = struct( ...
        ...
        "message", "", ...
        "bestScore", NaN, ...
        "improvementOverSeed", NaN, ...
        "returnedSeedBest", false, ...
...
function record = makeIterationRecord( ...
    ...
    record.primarySearch = selection.primarySearch;
